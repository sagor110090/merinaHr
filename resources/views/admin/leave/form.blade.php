...
<div class="form-group {{ $errors->has('date') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="date" type="date" id="date"  value="{{ isset($leave->date) ? $leave->date : ''}}" {{ (Hr::isAdmin()) ? 'readonly' : ''}} >
...
<div class="form-group {{ $errors->has('employee_id') ? 'has-error' : ''}}">
    ...
    <select name="employee_id" id="employee_id" class="select form-control">
        @foreach (Hr::findAll('employees') as $item)
            <option value="{{$item->id}}" {{ (Hr::isAdmin()) ? 'disable' : ''}} {{ ($leave->employee_id == $item->id) ? 'selected' : ''}}>{{$item->fname.' '.$item->lname}}</option>
        @endforeach
    </select>
...
<div class="form-group {{ $errors->has('application') ? 'has-error' : ''}}">
    ...
    <textarea name="application" id="application" cols="30" rows="10" class="form-control" {{ (Hr::isAdmin()) ? 'readonly' : ''}}>{{ isset($leave->application) ? $leave->application : ''}}</textarea>
...
<div class="form-group {{ $errors->has('file') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="file" type="file" id="file" value="{{ isset($leave->file) ? $leave->file : ''}}" {{ (Hr::isAdmin()) ? 'readonly' : ''}}>
    {!! $errors->first('file', '<p class="help-block">:message</p>') !!}
</div>
